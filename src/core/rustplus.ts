...
import { getAddress } from "@/utils/get-address.util";
...
export class RustPlus extends (EventEmitter as new () => TypedEventEmitter<RustPlusEvents>) {
        ...
        if (this.tokens.connection < RustPlus.MAX_REQUESTS_PER_IP_ADDRESS) {
            this.tokens.connection = Math.min(
                this.tokens.connection +
                    RustPlus.REQUESTS_PER_IP_REPLENISH_RATE,
        ...
        for (const playerId in this.tokens.playerId) {
            if (
                ...
                this.tokens.playerId[playerId] = Math.min(
                    this.tokens.playerId[playerId] +
                        RustPlus.REQUESTS_PER_PLAYER_ID_REPLENISH_RATE,
                    RustPlus.MAX_REQUESTS_PER_PLAYER_ID
                );
            }
        }
        ...
        if (
            ...
            this.tokens.serverPairing = Math.min(
                this.tokens.serverPairing +
                    RustPlus.REQUESTS_FOR_SERVER_PAIRING_REPLENISH_RATE,
    ...
    private async consumeTokens(
        playerId: string,
        tokens: number,
        waitForReplenish: boolean,
        timeoutMs: number = 10000
    ): Promise<ConsumeTokensError> {
    ...
    async connect(): Promise<boolean> {
        ...
        this.ws = new WebSocket(
            getAddress(this.useFacepunchProxy, this.ip, this.port)
        );
     ...
     *
     * @param {Omit<AppRequest, 'seq' | 'playerId' | 'playerToken'>} data - The data for the request,
     ...
     *
     * @returns {Promise<AppResponse | Error>} A Promise that resolves to the server's response if
            ...
            const result = this.sendRequest(
                ...
                (appMessage: AppMessage) => {
                    ...
                    try {
                        ...
                        const response: AppResponse = appMessage.response;
     ...
     *
     * @param {AppResponse} appResponse - The response object from the Rust server which may contain an error.
     *
     ...
     *                             present.
     */
    getAppResponseError(appResponse: AppResponse): AppResponseError {
     ...
     *
     * @returns {Promise<ConsumeTokensError | Error | AppResponse>} Returns a promise that resolves to the
    ...
    async getInfo(
        playerId: string,
        playerToken: number,
        waitForReplenish: boolean = true,
        timeoutMs: number = RustPlus.REQUEST_GET_INFO_TIMEOUT_MS
    ): Promise<AppResponse | Error | ConsumeTokensError> {
    ...
    async setEntityValue(
        ...
        value: boolean,
        waitForReplenish: boolean = true,
        timeoutMs: number = RustPlus.REQUEST_SET_ENTITY_VALUE_TIMEOUT_MS
    ): Promise<AppResponse | Error | ConsumeTokensError> {
    ...
    /**
     * Check the subscription status of an entity (Alarm).
    ...
    async checkSubscription(
        ...
        entityId: number,
        waitForReplenish: boolean = true,
        timeoutMs: number = RustPlus.REQUEST_CHECK_SUBSCRIPTION_TIMEOUT_MS
    ): Promise<AppResponse | Error | ConsumeTokensError> {
    ...
    /**
     * Set the subscription status of an entity (Alarm).
    ...
    async setSubscription(
        ...
        value: boolean,
        waitForReplenish: boolean = true,
        timeoutMs: number = RustPlus.REQUEST_SET_SUBSCRIPTION_TIMEOUT_MS
    ): Promise<AppResponse | Error | ConsumeTokensError> {
     ...
     *                                                                       is an issue with the request.
     */
    async cameraSubscribe(
        playerId: string,
        playerToken: number,
        identifier: string,
        waitForReplenish: boolean = true,
        timeoutMs: number = RustPlus.REQUEST_CAMERA_SUBSCRIBE_TIMEOUT_MS
    ): Promise<AppResponse | Error | ConsumeTokensError> {
    ...
    /**
     * Send camera input to the server (mouse movement).
    ...
    async cameraInput(
        ...
        y: number,
        waitForReplenish: boolean = true,
        timeoutMs: number = RustPlus.REQUEST_CAMERA_INPUT_TIMEOUT_MS
    ): Promise<AppResponse | Error | ConsumeTokensError> {
